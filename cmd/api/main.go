...
func main() {
	...
	// tenantMux holds every route that needs a resolved tenant (i.e.
	// almost everything). rootMux holds the handful of routes that must
	// work with NO tenant context: health checks (so they don't depend on
	// DNS/tenant setup) and the platform bootstrap endpoint (which CREATES
	// the first tenant, so obviously can't require one already existing).
	...
	// Badge scan lookup at doors — any authenticated security/reception
	// role with visits.view can verify a badge; no separate permission
	// needed since it's read-only and already tenant + token scoped.
	...
	// Gate actions: "issue" = check out (item/visitor leaving), "verify" =
	// check in (item/visitor returning), matching the permission names in
	// the spec's settings.permissions list.
	...
	// rootMux (health checks, platform bootstrap) is layered on top,
	// unwrapped — it must work with no tenant resolved at all.
