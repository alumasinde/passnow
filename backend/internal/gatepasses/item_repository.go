...
func (r *ItemRepository) CreateForGatepass(ctx context.Context, tx *sql.Tx, tenantID, gatepassID int64, items []ItemInput) error {
	for _, it := range items {
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO gatepass_items
				(gatepass_id, tenant_id, name, description, category, quantity, unit,
				 serial_number, asset_number, item_condition, direction, created_at, updated_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
			gatepassID, tenantID, it.Name, it.Description, it.Category, it.Quantity, it.Unit,
...
func (r *ItemRepository) ListForGatepass(ctx context.Context, tenantID, gatepassID int64) ([]ItemDTO, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, name, description, category, quantity, unit, serial_number, asset_number, item_condition, direction
		FROM gatepass_items WHERE gatepass_id = ? AND tenant_id = ? ORDER BY id`, gatepassID, tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
