...
$widgetRoot = dirname(__DIR__, 2) . '/templates/dashboard/widgets';
?>
...
<?php if ($statWidgets !== []): ?>
    <section class="dashboard-grid dashboard-stats-grid">
        <?php foreach ($statWidgets as $widget): ?>
            <?php require $widgetRoot . '/stat.php'; ?>
        <?php endforeach; ?>
    </section>
<?php endif; ?>
...
<?php if ($contentWidgets !== []): ?>
    <section class="dashboard-grid dashboard-content-grid">
        <?php foreach ($contentWidgets as $widget): ?>
            ...
            $widgetFile = $widgetRoot . '/' . $type . '.php';
            if (is_file($widgetFile)) require $widgetFile;
            ?>
        <?php endforeach; ?>
    </section>
