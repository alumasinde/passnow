...
<form method="post" class="content-card form-card" data-loading-form>
 ...
 <div class="form-grid">
  ...
  <div class="field"><label for="department_id">Department</label><select id="department_id" name="department_id"><option value="">No department</option><?php $selectedDepartment=(string)oldOr('department_id',$currentDepartmentID!==null?(string)$currentDepartmentID:'');foreach($departments as $d):$did=(int)($d['id']??0);?><option value="<?=e((string)$did)?>" <?=($selectedDepartment===(string)$did)?'selected':''?>><?=e((string)($d['name']??''))?></option><?php endforeach;?></select><?php if($currentDepartmentID):?><small class="field-help">Preselected from your user department.</small><?php endif;?></div>
  <div class="field"><label>Requester type *</label><select name="requester_type" required data-requester-type><option value="employee" <?=((string)oldOr('requester_type','employee')==='employee')?'selected':''?>>My employee gatepass</option><option value="visitor" <?=((string)oldOr('requester_type')==='visitor')?'selected':''?>>Visitor</option></select><small class="field-help">Employee requests are tied to the signed-in user.</small></div>
  ...
  <div class="field field-full"><label>Purpose *</label><textarea name="purpose" required maxlength="255" placeholder="Explain why this gatepass is required"><?=e((string)oldOr('purpose'))?></textarea></div>
