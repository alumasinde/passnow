...
<form method="post" class="content-card form-card" data-loading-form>
 ...
 <div class="form-grid">
  ...
  <div class="field"><label>Department</label><select name="department_id"><option value="">No department</option><?php foreach($departments as $d): ?><option value="<?=e((string)($d['id']??''))?>" <?=((string)oldOr('department_id')===(string)($d['id']??''))?'selected':''?>><?=e((string)($d['name']??''))?></option><?php endforeach;?></select></div>
