...
$errors=[];$roles=[];$departments=[];
try{$p=Auth::api(App::api(),'GET','/api/v1/departments');$departments=apiRows($p);}catch(Throwable){}
try{$p=Auth::api(App::api(),'GET','/api/v1/roles');$roles=apiRows($p);if(!$roles)$errors[]='No roles are available. Create a role before creating a user.';}catch(ApiException $e){$errors[]='Unable to load roles: '.$e->getMessage();}catch(Throwable){$errors[]='Unable to load roles.';}
if(requestMethod()==='POST'){
 Csrf::requireValid($_POST['_csrf']??null);
 $payload=['email'=>trim((string)($_POST['email']??'')),'first_name'=>trim((string)($_POST['first_name']??'')),'last_name'=>trim((string)($_POST['last_name']??'')),'role_id'=>(int)($_POST['role_id']??0),'department_id'=>(int)($_POST['department_id']??0)?:null];
