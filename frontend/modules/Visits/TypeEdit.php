...
if($id){try{$p=Auth::api(App::api(),'GET','/api/v1/visit-types/'.$id);$item=apiValue($p,'item',$p['data']??$p);if(!is_array($item))$item=[];}catch(Throwable $e){$errors[]=$e instanceof ApiException?$e->getMessage():'Unable to load record.';}}
if(requestMethod()==='POST'){
 Csrf::requireValid($_POST['_csrf']??null);
 $payload=[];
 $payload['name']=trim((string)($_POST['name']??''));$payload['description']=trim((string)($_POST['description']??''));
 if(!$errors)try{
  if($id){ConfigCrud::update('/api/v1/visit-types/'.$id,$payload);flash('success','Updated successfully.');}
  else{ConfigCrud::create('/api/v1/visit-types',$payload);flash('success','Created successfully.');}
