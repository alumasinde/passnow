...
func RegisterAPI(mux *http.ServeMux, api *API) {
	...
	mux.Handle("POST /api/v1/visits/{id}/qr/issue", protected("visit.create", api.VisitHandler.IssueQR))
	mux.Handle("POST /api/v1/visits/{id}/qr/invalidate", protected("visit.cancel.all", api.VisitHandler.InvalidateQR))
	...
	mux.Handle("GET /api/v1/visit-badges/{token}", protected("visit.read.all", api.VisitHandler.BadgeLookup))
