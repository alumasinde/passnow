...
func TestVisitRepositoryCheckInCheckOutAndMovements(t *testing.T) {
	...
	for _, table := range []string{"visitors","id_types","visits","visit_movements","gates","users"} { testutil.RequireTable(t, db, table) }
	...
	if err:=repo.RecordMovement(ctx,visitID,MovementCheckIn,userID,MovementInput{GateID:gateID});err!=nil{t.Fatalf("RecordMovement check-in: %v",err)}
