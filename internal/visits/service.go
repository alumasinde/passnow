...
func(s *Service)CheckInAtGate(ctx context.Context,tenantID,id,actorUserID int64,in MovementInput)(*Visit,error){
    if in.GateID==0{return nil,ErrMovementGateRequired};var active,allows bool;if err:=s.repo.DB().QueryRowContext(ctx,"SELECT active,allows_entry FROM gates WHERE id=? AND deleted_at IS NULL",in.GateID).Scan(&active,&allows);err!=nil||!active{return nil,ErrMovementGateInvalid};if !allows{return nil,ErrMovementGateDirection};if err:=s.validateMovementDevice(ctx,in);err!=nil{return nil,err}
    ...
    v,err:=s.repo.CheckIn(ctx,id,actorUserID);if err!=nil{return nil,err};if err:=s.repo.RecordMovement(ctx,id,MovementCheckIn,actorUserID,in);err!=nil{return nil,err};s.audit(ctx,tenantID,actorUserID,ActionVisitCheckedIn,id,map[string]any{"badge_number":v.BadgeNumber});return v,nil
}
...
func(s *Service)CheckOutAtGate(ctx context.Context,tenantID,id,actorUserID int64,in MovementInput)(*Visit,error){
    if in.GateID==0{return nil,ErrMovementGateRequired};var active,allows bool;if err:=s.repo.DB().QueryRowContext(ctx,"SELECT active,allows_exit FROM gates WHERE id=? AND deleted_at IS NULL",in.GateID).Scan(&active,&allows);err!=nil||!active{return nil,ErrMovementGateInvalid};if !allows{return nil,ErrMovementGateDirection};if err:=s.validateMovementDevice(ctx,in);err!=nil{return nil,err}
    v,err:=s.repo.CheckOut(ctx,id,actorUserID);if err!=nil{return nil,err};if err:=s.repo.RecordMovement(ctx,id,MovementCheckOut,actorUserID,in);err!=nil{return nil,err};s.audit(ctx,tenantID,actorUserID,ActionVisitCheckedOut,id,nil);return v,nil
}
