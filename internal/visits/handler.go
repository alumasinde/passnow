...
func (h *Handler) QRLookup(w http.ResponseWriter,r *http.Request){tenant,ok:=reqctx.TenantFromContext(r.Context());if !ok{httpx.WriteError(w,httpx.ErrAuthRequired);return};token:=r.PathValue("token");if token==""{httpx.WriteError(w,httpx.ErrNotFound);return};v,visitor,err:=h.svc.QRLookup(r.Context(),tenant.ID,token);if err!=nil{writeServiceError(w,err);return};d:=h.svc.ToDTO(r.Context(),v);d.VisitorName=visitor.FullName();httpx.WriteJSON(w,http.StatusOK,d)}
