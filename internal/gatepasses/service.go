...
func (s *Service) audit(ctx context.Context, tenantID, actorUserID int64, action string, entityID int64, metadata map[string]any) { _ = s.auditRepo.Record(ctx, s.auditRepo.DB(), audit.Entry{TenantID:&tenantID, ActorUserID:&actorUserID, Action:action, EntityType:"gatepass", EntityID:&entityID, Metadata:metadata}) }
