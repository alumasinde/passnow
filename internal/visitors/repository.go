...
type ListFilter struct{Status *Status;CompanyID *int64;Search string}
...
func (r *Repository) List(ctx context.Context,f ListFilter,p httpx.Pagination)([]Visitor,int,error){
	...
	if f.Search!=""{where+=" AND (id_number=? OR first_name LIKE ? OR last_name LIKE ?)";like:=f.Search+"%";args=append(args,f.Search,like,like)}
